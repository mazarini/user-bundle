...
{
    protected static $defaultName = 'package:file';
    ...
    protected function configure()
    {
        ...
        ->setDefinition([
            ...
            ->setDescription('List files from recipes')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($packages as $package) {
            foreach ($package->getFiles() as $file) {
                $lines[] = [$file['name'], $file['delete'], $package->getName()];
            }
        }
        usort($lines, function ($a, $b) { return strcmp($a[0], $b[0]); });
        $table = new table($output);
        $table->setHeaders(['File', 'delete', 'package']);
        $table->setRows($lines);
        $table->render();
